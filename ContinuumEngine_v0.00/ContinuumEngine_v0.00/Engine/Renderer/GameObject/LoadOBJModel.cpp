...
void LoadOBJModel::LoadModel(const std::string& fileName_, const std::string& matName_, bool isGlowing_) {
...
void LoadOBJModel::LoadMaterial(const std::string& fileName_) {
	...
	if (currentTexture == 0) {
		//
		TextureHandler::GetInstance()->CreateTexture(fileName_, "Engine/GlobalElements/Resources/Textures/" + fileName_ + ".jpg", isGlowing);
		currentTexture = TextureHandler::GetInstance()->GetTexture(fileName_);
	}
}
